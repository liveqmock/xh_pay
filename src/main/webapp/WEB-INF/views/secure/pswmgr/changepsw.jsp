...
<body>
    ...
    <div class="paycontent">
       ...
       <div class="safe-con">
			...
			<div class="mail-con">
				<form action="<c:url value="/pswmgr/changepsw" />" id="email-form" method="post">
					<div class="reset-con">
						<p><span class="reset-tilte reset-psdcon">旧的支付密码：</span><input id="old-psd" name="oldpsw" type="password"/></p>
						<p><span class="reset-tilte reset-psdcon">新的支付密码：</span><input id="new-psd" name="newpsw" type="password"/><span class="psd-tips">(请输入8位或以上字符)</span></p>
						<p><span class="reset-tilte reset-psdcon">确认支付密码：</span><input id="new-psdconf" name="renewpsw" type="password"/><span class="psd-tips">(请输入8位或以上字符)</span></p>
		...
		<script type="text/javascript">
			KISSY.use('node,xh/validator',function(S,Node,Validator){
				...
				validator.add('#new-psdconf',{
					...
					equalTo: ['#new-psd','两次输入不一致。'] //重复输入
				});
				validator.add('#new-psd',{
					required: true,
					minLength : [8,'密码不能少于8位']
				});
				validator.add('#old-psd',{
					required: true
// 					minLength : [8,'密码不能少于8位']
				});
			});
